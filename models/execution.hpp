...
inline void add_profiling_info(const sycl::event& e) {
    try {
        ...
        g_total_internal_kernel_ms += (double)(end - start) / 1000000.0;
    } catch (...) {}
}

inline void add_profiling_info(const std::vector<sycl::event>& events) {
    for (const auto& e : events) add_profiling_info(e);
}
        ...
        if (on_device) {
            ...
            if (split_index < kernels.size()) {
                ...
                for (int i = split_index; i < kernels.size(); ++i) {
                    ...
                    all_events.insert(all_events.end(), deps.begin(), deps.end());
                }
                // Wait for fallback kernels
                for (auto &e : all_events) e.wait();
