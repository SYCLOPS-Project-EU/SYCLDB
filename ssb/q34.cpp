...
void build_hashtable_d(int *filter_col, int *dim_key, int *dim_val,
                       int num_tuples, int *hash_table, int num_slots,
                       int val_min, const sycl::nd_item<1> &item_ct1) {
  int items[ITEMS_PER_THREAD];
  int items2[ITEMS_PER_THREAD];
  int selection_flags[ITEMS_PER_THREAD];
  ...
  BlockPredEQ<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 199712, selection_flags, num_tile_items, item_ct1);
...
int main(int argc, char **argv) {
  ...
  build_tables[2].h_filter_col =
      loadColumn<int>("d_yearmonthnum", D_LEN, cpu_queue);
      ...
      cgh.parallel_for(
          sycl::nd_range<1>(gws, lws), [=](sycl::nd_item<1> item_ct1) {
            build_hashtable_d<N_BLOCK_THREADS, N_ITEMS_PER_THREAD>(
                ...
                hash_table, build_tables_num_slots_ct5, 19920101, item_ct1);
          });
    });
  };
  ...
  prob.probe_function = [&](int **probe_data, int partition_len,
    ...
    if (modes == 1 || modes == 2) {
      ...
      std::cout << "Q34 >>Internal total timer reported " << total_time
                << " ms.\n";
      std::cout << "Q34 >>External total timer reported "
