...
void probe(int *lo_orderdate, int *lo_partkey, int *lo_custkey, int *lo_suppkey,
  ...
  probe_keys_element(lo_partkey[idx], sf, p_len, ht_p, p_len, 0);
...
int main(int argc, char **argv) {
  ...
  build_tables[0].h_filter_col = loadColumn<int>("p_mfgr", P_LEN, cpu_queue);
  ...
  build_tables[1].h_filter_col = loadColumn<int>("s_region", S_LEN, cpu_queue);
  ...
  build_tables[2].h_dim_val = loadColumn<int>("c_nation", C_LEN, cpu_queue);
  ...
  prob.h_lo_data[1] = loadColumn<int>("lo_custkey", LO_LEN, cpu_queue);
  prob.h_lo_data[2] = loadColumn<int>("lo_partkey", LO_LEN, cpu_queue);
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(partition_len, [=](sycl::id<1> id) {
            probe(probe_data_ct0, probe_data_ct1, probe_data_ct2,
                  ...
                  build_tables_num_slots_ct14, res, id);
          });
      });
    }
    if (modes == 1 || modes == 2) {
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(partition_len, [=](sycl::id<1> idx) {
          probe_keys_element(probe_data_ct3[idx], selection_flags[idx],
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        int build_tables_num_slots_ct12 = build_tables[2].num_slots;
        cgh.parallel_for(partition_len, [=](sycl::id<1> idx) {
          probe_keys_vals_element(probe_data_ct2[idx], c_nation[idx],
                                  selection_flags[idx],
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        int build_tables_num_slots_ct8 = build_tables[0].num_slots;
        cgh.parallel_for(partition_len, [=](sycl::id<1> idx) {
          probe_keys_element(probe_data_ct1[idx], selection_flags[idx],
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(partition_len, [=](sycl::id<1> idx) {
          if (selection_flags[idx]) {
            int hash = (c_nation[idx] * 7 + (year[idx] - 1992)) %
                       ((1998 - 1992 + 1) * 25);
            res[hash * 4] = year[idx];
            res[hash * 4 + 1] = c_nation[idx];
      ...
      std::cout << "Q41 >>Internal total timer reported " << total_time
                << " ms.\n";
      std::cout << "Q41 >>External total timer reported "
