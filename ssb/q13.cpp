...
DeviceSelectIf(int *lo_orderdate, int *lo_discount, int *lo_quantity,
  ...
  bool sf = false;
  selection_element<int>(sf, lo_orderdate[idx], NONE, GE, 19940204);
  selection_element<int>(sf, lo_orderdate[idx], AND, LE, 19940210);
  selection_element<int>(sf, lo_quantity[idx], AND, GE, 26);
  selection_element<int>(sf, lo_quantity[idx], AND, LE, 35);
  selection_element<int>(sf, lo_discount[idx], AND, GE, 5);
  selection_element<int>(sf, lo_discount[idx], AND, LE, 7);
  if (sf)
...
DeviceSelectIf(int *lo_orderdate, int *lo_discount, int *lo_quantity,
  ...
  BlockPredGTE<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 19940204, selection_flags, num_tile_items, item_ct1);
  BlockPredAndLTE<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 19940210, selection_flags, num_tile_items, item_ct1);
  ...
  BlockPredAndGTE<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 5, selection_flags, num_tile_items, item_ct1);
  BlockPredAndLTE<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 7, selection_flags, num_tile_items, item_ct1);
    ...
    event = queue.submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for(sycl::nd_range<1>(gws, lws), sum_reduction,
        ...
        cgh.parallel_for(partition_len, sum_reduction,
          ...
          DeviceSelectIf(probe_data_ct0, probe_data_ct1, probe_data_ct2, probe_data_ct3,
