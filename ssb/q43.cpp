...
void probe(int *lo_orderdate, int *lo_partkey, int *lo_custkey, int *lo_suppkey,
  ...
  int year;
  bool sf = true;
  probe_keys_vals_element(lo_suppkey[idx], s_city, sf, s_len, ht_s, s_len, 0);
  ...
  probe_keys_vals_element(lo_orderdate[idx], year, sf, d_len, ht_d, d_len,
                          19920101);
  if (sf) {
    int hash = ((year - 1992) * 250 * 1000 + s_city * 1000 + brand) %
...
int main(int argc, char **argv) {
  ...
  prob.probe_function = [&](int **probe_data, int partition_len,
    ...
    if (modes == 1 || modes == 2) {
      int *s_city = sycl::malloc_shared<int>(partition_len, queue);
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        int build_tables_num_slots_ct10 = build_tables[1].num_slots;
        cgh.parallel_for(partition_len, [=](sycl::id<1> idx) {
          probe_keys_vals_element(probe_data_ct3[idx], s_city[idx],
                                  selection_flags[idx],
      ...
      event = queue.submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(partition_len, [=](sycl::id<1> idx) {
          if (selection_flags[idx]) {
            int hash = ((year[idx] - 1992) * 250 * 1000 + s_city[idx] * 1000 +
                        brand[idx]) %
                       ((1998 - 1992 + 1) * 250 * 1000);
            res[hash * 6] = year[idx];
            res[hash * 6 + 1] = s_city[idx];
            res[hash * 6 + 2] = brand[idx];
      ...
      std::cout << "Q43 >>Internal total timer reported " << total_time
                << " ms.\n";
      std::cout << "Q43 >>External total timer reported "
