...
template <int BLOCK_THREADS, int ITEMS_PER_THREAD>
SYCL_EXTERNAL void build_hashtable_p(int *dim_key, int *dim_val, int num_tuples,
  ...
  BlockPredGTE<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 260, selection_flags, num_tile_items, item_ct1);
  BlockPredAndLTE<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items, 267, selection_flags, num_tile_items, item_ct1);
...
int main(int argc, char **argv) {
  ...
  prob.res_size = ((1998 - 1992 + 1) * 1000);
  ...
  prob.probe_function = [&](int **probe_data, int partition_len,
    ...
    if (modes == 1 || modes == 2) {
      ...
      std::cout << "Q22 >>Internal total timer reported " << total_time
                << " ms.\n";
      std::cout << "Q22 >>External total timer reported "
