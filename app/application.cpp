...
struct QueryRunSummary
{
    ...
    std::vector<double> internal_kernel_ms;
    ...
    std::ostream &perf_out)
{
    ...
    if (runtime.config.reuse_allocators_across_repetitions)
    {
        ...
        for (int i = 0; i < PERFORMANCE_REPETITIONS; ++i)
        {
            ...
            summary.internal_kernel_ms.push_back(g_total_internal_kernel_ms);
            ...
            std::cout << "Repetition " << i + 1 << "/" << PERFORMANCE_REPETITIONS
                << " - " << exec_time.count() << " ms - "
                << total_time.count() << " ms - "
                << "JIT: " << g_total_jit_ms << " ms, Kernel: " << g_total_kernel_ms << " ms (InternalTimeKernel: " << g_total_internal_kernel_ms << " ms), Parse: " << parse_time.count() << " ms" << std::endl;
        }
    }
    else
    {
...
int run_app(int argc, char **argv)
{
    try
    {
        ...
        runtime_setup::RuntimeEnvironment runtime =
            runtime_setup::build_runtime_environment(options.device_selector);
