...
AppOptions parse_app_options(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--device-selector")
        {
            if (!requires_value(i, argc))
                throw std::invalid_argument("--device-selector requires a value");
...
void print_help(std::ostream &out, const char *program_name)
{
    out
        ...
        << "  --data-dir <path>        SSB columnar data path\n"
        << "  --device-selector <sel>  Device selector substring (e.g. cpu, cuda, opencl)\n"
