...
std::vector<sycl::device> discover_execution_devices(const std::string &selector)
{
    std::vector<sycl::device> devices = sycl::device::get_devices();
    if (!selector.empty())
    {
        std::vector<sycl::device> filtered;
        for (const auto &d : devices)
        {
            std::string name = d.get_info<sycl::info::device::name>();
            std::string platform = d.get_info<sycl::info::device::platform>().get_info<sycl::info::platform::name>();
            std::string backend_str = backend_to_string(d.get_backend());
            auto lower = [](std::string s) {
                std::transform(s.begin(), s.end(), s.begin(), ::tolower);
                return s;
            };
            std::string s_lower = lower(selector);
            if (lower(name).find(s_lower) != std::string::npos ||
                lower(platform).find(s_lower) != std::string::npos ||
                lower(backend_str).find(s_lower) != std::string::npos)
            {
...
RuntimeEnvironment build_runtime_environment(const std::string &device_selector)
{
    ...
    const std::vector<sycl::device> execution_devices = discover_execution_devices(device_selector);
    ...
    for (const sycl::device &device : execution_devices)
    {
        ...
        runtime.queues.device_queues.emplace_back(
            device,
