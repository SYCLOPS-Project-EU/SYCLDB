...
inline void BlockStoreDirect(int tid, T *block_itr,
...
template <typename T, int BLOCK_THREADS, int ITEMS_PER_THREAD>
inline void BlockStore(T *out, T (&items)[ITEMS_PER_THREAD], int num_items,
                       const sycl::nd_item<1> &item_ct1) {
  T *block_itr = out;

  if ((BLOCK_THREADS * ITEMS_PER_THREAD) == num_items) {
    BlockStoreDirect<T, BLOCK_THREADS, ITEMS_PER_THREAD>(
